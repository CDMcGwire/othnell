...
onMount(() => {
  for (let label of labels)
    for (let tieredStyle of tieredStyles) tieredStyle.apply(label, depth)
})
...
<div class="nav-tree">
  {#each routes as route, i (route.name)}
    <div
      class="nav-tree-label nav-tree-label-{depth}"
    ...
    >
      ...
      <a
        ...
        on:keyup|stopPropagation="{e => {
          if (!e.isComposing && e.key === ' ') route.open = !route.open
        }}"
      >
        {route.name}
      </a>
